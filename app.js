document.addEventListener('DOMContentLoaded', function() {
    ...
    function pImg(svc){var c={youtube:'#ff0000',spotify:'#1db954',apple:'#fc3c44',yandex:'#ffcc00'},i={youtube:'▶',spotify:'●',apple:'♫',yandex:'♪'};var svg='<svg xmlns="http://www.w3.org/2000/svg" width="64" height="64"><rect fill="'+(c[svc]||'#555')+'" width="64" height="64" rx="8"/><text fill="white" x="32" y="36" text-anchor="middle" font-size="20">'+(i[svc]||'?')+'</text></svg>';return'data:image/svg+xml,'+encodeURIComponent(svg)}
    ...
    // Голосование
    async function voteFor(side){var id=side==='left'?duelLeftId:duelRightId;if(!id||id==='local'){toast('Нельзя голосовать за локальный тир-лист');return}if(!db){toast('Голосование недоступно');return}showLoading(true);try{var ref=db.collection('tierlists').doc(id);var doc=await ref.get();if(doc.exists){var cw=doc.data().wins||0;await ref.update({wins:cw+1});toast('Голос учтён! Всего побед: '+(cw+1));document.getElementById('duelVoteBar').classList.remove('show');duelLeftId=null;duelRightId=null}}catch(e){toast('Ошибка голосования')}showLoading(false)}
        ...
        setTimeout(function(){
            document.querySelectorAll('.tier-items').forEach(function(el){
                ...
                new Sortable(el,{
                    ...
                    onEnd:function(evt){
                        var fromList=parseInt(evt.from.dataset.listNum,10);
                        var toList=parseInt(evt.to.dataset.listNum,10);
                        var fromTier=parseInt(evt.from.dataset.tierIndex,10);
                        var toTier=parseInt(evt.to.dataset.tierIndex,10);
                        if(isNaN(fromList)||isNaN(toList)||isNaN(fromTier)||isNaN(toTier))return;
                        var fromData=(fromList===1)?data1:data2;
                        var toData=(toList===1)?data1:data2;
                        pushH(fromList);
                        if(fromList!==toList)pushH(toList);
                        var movedItem=fromData[fromTier].items.splice(evt.oldIndex,1)[0];
                        toData[toTier].items.splice(evt.newIndex,0,movedItem);
                        sD(fromList,fromData);
                        if(fromList!==toList)sD(toList,toData);
    ...
    function updateUI(){document.body.classList.toggle('editing',editing);var eb=document.getElementById('editBtn');eb.innerHTML=editing?'<i data-lucide="check"></i> Готово':'<i data-lucide="edit-3"></i> Редактировать';if(editing)eb.classList.add('primary');else eb.classList.remove('primary');document.getElementById('resetBtn').classList.toggle('hidden',!editing);document.getElementById('addTierBtn').classList.toggle('hidden',!editing);document.getElementById('col2').style.display=compare?'block':'none';lucide.createIcons()}
    ...
    async function loadFromURL(){var p=new URLSearchParams(location.search);if(p.has('id')&&db){try{var doc=await db.collection('shared').doc(p.get('id')).get();if(doc.exists){var d=JSON.parse(doc.data().data);if(Array.isArray(d)){pushH(1);data1=d;sD(1,data1);ctid=p.get('id');if(p.has('tierlistId'))ctid=p.get('tierlistId');await loadComments(ctid);history.replaceState({},'',location.pathname);return true}}}catch(e){}}if(p.has('data')){try{var d=JSON.parse(LZString.decompressFromEncodedURIComponent(p.get('data')));if(Array.isArray(d)){pushH(1);data1=d;sD(1,data1);if(p.has('tierlistId')){ctid=p.get('tierlistId');await loadComments(ctid)}history.replaceState({},'',location.pathname);return true}}catch(e){}}return false}
    ...
    function bindEvents(){
        ...
        document.getElementById('publishBtn').addEventListener('click',async function(){if(!db){toast('Недоступно');return}if(!data1||data1.length===0||data1.every(function(t){return t.items.length===0})){toast('Тир-лист пуст!');return}var name=prompt('Название:','Мой тир-лист');if(!name)return;showLoading(true);try{var docRef=await db.collection('tierlists').add({name:escapeHTML(name),data:JSON.stringify(data1),trackCount:data1.reduce(function(s,t){return s+t.items.length},0),wins:0,createdAt:firebase.firestore.FieldValue.serverTimestamp()});ctid=docRef.id;toast('Опубликовано!');document.getElementById('galleryModal').classList.remove('open')}catch(e){toast('Ошибка публикации')}showLoading(false)});
        ...
        document.getElementById('closePlayer').addEventListener('click',function(){document.getElementById('playerModal').classList.remove('open');document.getElementById('playerFrame').src=''});
        ...
        document.querySelectorAll('.modal').forEach(function(m){m.addEventListener('click',function(e){if(e.target===this&&this.id!=='neonModal')this.classList.remove('open')})});
    }
